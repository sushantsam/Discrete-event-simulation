...
def lcgrand(Stream):
    ...
    MULT1 = 24112
    MULT2 = 26143
    zi = ZRNG[Stream-1]  # retrieves the current element of the sequence for the given Stream
    ...
    if zi < 0:
        zi = zi + MODLUS
    ZRNG[Stream-1] = zi # advances the sequence
    lcgrand = (zi // 128 | 1) / 16777216.0 # returns uniform random variable
    return lcgrand
...
def Erlang2(m, Mean, Stream):
    mean_exponential = float(Mean)
    Sum = 0.0
    for i in range(0,m,1):
        Sum = Sum + Expon(mean_exponential, Stream)
    erlang = Sum
